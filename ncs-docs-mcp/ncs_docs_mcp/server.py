"""FastMCP server exposing the NCS 1.6.1 docs via hybrid retrieval.

Pointer-first: ``search_docs`` returns *locations* (repo / file / anchor /
breadcrumb / line range) plus a snippet, so the agent can cheaply locate a
section and then ``Read`` the real RST for exactness. Full text is available on
demand via ``get_section`` / ``get_doc``; the xref graph via ``related``.
"""
...
def search_docs(
    ...
    """Search the NCS 1.6.1 docs and return locations (not full text).
...
def _open_index(index_path: Path) -> sqlite3.Connection:
    db = sqlite3.connect(f"file:{index_path.as_posix()}?mode=ro", uri=True, check_same_thread=False)
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="NCS 1.6.1 docs hybrid MCP server")
    ap.add_argument("index", nargs="?",
                    default=str(Path(__file__).resolve().parent.parent / "index.sqlite"),
                    help="path to index.sqlite")
    args = ap.parse_args()

    index_path = Path(args.index).resolve()
    if not index_path.is_file():
        sys.exit(f"index not found: {index_path}\nBuild it first: uv run python build_index.py")

    _DB = _open_index(index_path)
    rel = _DB.execute("SELECT value FROM meta WHERE key = 'docs_root_relative'").fetchone()
    _DOCS_ROOT = (index_path.parent / (rel[0] if rel else "../ncs-1.6.1-docs")).resolve()

    model = _DB.execute("SELECT value FROM meta WHERE key = 'embed_model'").fetchone()
    if model and model[0] != EMBED_MODEL:
        print(f"warning: index built with {model[0]}, server expects {EMBED_MODEL}",
              file=sys.stderr)
