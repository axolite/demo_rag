...
def open_corpus(index_path: Path) -> Corpus:
    ...
    relative to the index file at build time), so a clone works from anywhere.
    Raises if that meta key is missing — there is no SDK-specific fallback.
    """
    db = sqlite3.connect(
    ...
    return Corpus(db=db, docs_root=docs_root, embed_model=read_meta(db, "embed_model"))
